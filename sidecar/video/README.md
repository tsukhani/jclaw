...
404 {unknown_job}, `GET /jobs/<id>/result` -> mp4 | 409 {not_ready}, `POST /pull` -> ndjson progress (WAN engines) | 400 {pull_unsupported} for LTX engines, which fetch their weights on first load.
