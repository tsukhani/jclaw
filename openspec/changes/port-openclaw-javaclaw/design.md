...
**Non-Goals (v0.1.0):**
See proposal.md "Future Capabilities" for the full audit of gaps vs OpenClaw/JavaClaw.
- Memory auto-capture (high priority for v0.2.0 — agent has no long-term learning without it)
- Channel DM policy / access control (high priority — security gap)
- Concurrent message handling / queue mode (high priority — correctness under load)
- Media message handling (medium — non-text messages silently ignored)
- Onboarding wizard (medium — requires manual navigation)
- Multiple accounts per channel type (medium)
