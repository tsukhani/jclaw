...
- A task fire always runs on its agent's current model: `Task.modelProvider`/`modelId` are stored and `task_manager` advertises the override, but `AgentRunner.runTaskTurn` never applies them — a known bug, not the design.
...
enforces it at the request layer from `AuthCheck` and `LoadtestAuthCheck` — Play assigns
