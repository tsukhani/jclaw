...
- Rules with their reasons: the sections below. Settled verdicts before re-proposing work: `docs/spikes/` — gitignored, so it holds only what this clone wrote; verdicts filed before that are in git history under `docs(spike`.
...
- One implementation each, after JCLAW-1231, so a fix lands once: `jobs.RetentionDays` (all three cleanup windows), `AttachmentService.toView`/`toViews` (attachment JSON on the SSE frame and the REST read alike), `channels.TelegramInboundTurn` (the webhook and the poller run one turn), `SubagentSpawnArgs.batchChild`, `LocalSidecarDaemon.singleFlight` (every sidecar manager — `lock()` is gone), and `ConfigService.isDispatcherCapKey` on both the set and the delete arm. Grepping `Mirrors|Same as|kept in sync|must match` is a weak proxy for duplication here: 95 of the 96 hits are deliberate cross-references — per-channel wire shapes, validation strings, query-technique pointers — so read for the twin rather than grep for it.
...
- Test classes run concurrently in one JVM: never flip a process-global without its lock (`LuceneTestSync`, `ShellSandboxSync`, `ToolRegistrySync`, `TelemetryTestSync`, `LoadTestHarnessSync`, `SlackWebApiTestSync`, `JevBreakerTestSync`); scope shared-table assertions to your own rows; a "seeded row missing" red is usually the `Fixtures.deleteDatabase` race, not a regression. Circuit breakers live in a process-global registry keyed by provider or MCP server name, so a test that drives `chat()`, a stream or `callTool` mints its own name and `CircuitBreakers.remove`s it — never `test-provider`, which another class may be tripping. JEV's breaker is the exception: its name is fixed (`decision:jev`), so a test that sends a JEV request holds `JevBreakerTestSync`, which drops the breaker as it locks and unlocks.
...
- JClaw has two trust axes. **The agent principal:** `findById` without owner scoping is not IDOR. `/api/providers`, the config reads and the channel list are agent-reachable on purpose — remediate there at the seam (SsrfGuard, masking, the dangerous-verb gate), never by blocking the endpoint. Since JCLAW-1270 the boundary is one annotation rather than four mechanisms: **every routed `/api` action declares `@AgentAccess`**, `AgentAccessGate` enforces it from `AuthCheck` and `LoadtestAuthCheck`, and `JClawApiTool` derives what it advertises from the same declaration — so hiding a route and refusing it can no longer drift apart. The three levels are `OPEN`, `OWN_ONLY` and `OPERATOR_ONLY`, **and the absence of the annotation means `OPERATOR_ONLY`**: the gate was default-allow until JCLAW-1270, so a new endpoint was reachable the moment it was written and the omission looked exactly like a decision. `@ChatHidden`, `@AgentCallable` and `JClawApiTool.PATH_BLOCKLIST` are gone, and with them the per-controller `requireOperator()` helpers — an `OPERATOR_ONLY` action needs no guard in its body, because the interceptor refuses before it runs. Two rules keep it honest: `CapabilityRulesTest.everyRoutedApiActionDeclaresAnAgentAccessLevel` fails the build on an action with no level, on an `OPEN`/`OWN_ONLY` mutating route whose mandatory `reason` names nothing that bounds it, and on an `OWN_ONLY` action that reaches no ownership check; and `JClawApiToolTest.theCallableApiSurfaceIsExactlyTheseRoutes` pins the 138 routes an agent can invoke, so widening the surface is a visible diff in two places. Reads are in scope now — they were not before JCLAW-1270, which left 91 GET routes answering to no rule at all. `CredentialReadGateConformanceTest` additionally fails the build when a `GET` route's declared `@ApiResponse` type carries a String component named like a credential — expose a `hasX` boolean instead, as the channel binding views do; it reads the Swagger declaration because no bytecode rule can follow a field through Gson, so the 34 GET routes that declare no response type are unchecked and ratcheted rather than swept (JCLAW-1268). **Which agent is calling is now knowable.** One token serves every agent, so the credential cannot say; `JClawApiTool` stamps `X-JClaw-Agent-Id` from the `Agent` it is handed and `RequestPrincipal.callingAgent()` reads it back, honoured only on an agent-originated request. The model never builds the request, so it cannot forge the header, and an unstamped bearer reads as an unidentified agent — which `OWN_ONLY` refuses. It stamps `X-JClaw-Caller-Origin` the same way — the calling turn's `DangerousActionGate.effectiveOrigin` — so `POST`/`PATCH /api/tasks` record and lower a task's fire-time origin by the turn behind the call, the rule `task_manager` applies; only the operator's `POST /api/tasks/{id}/trust` raises one. `mayReachAgentScopedRow` is the row check and `isOperatorOrMainAgent` the caller check: the operator and `main` reach everything, every other agent only its own rows. `main` is the operator's assistant and supervises the others, so the asymmetry is deliberate — it sees down the tree, nothing sees up or across. That scoping covers conversations and their messages, and subagent runs by `parentAgent`; a list route pins its query to the caller rather than refusing a row it would have had to read first. Give an agent a narrow tool argument rather than reopening one of those — the printer tool is the worked example (JCLAW-1229): `host`/`port` stay open and `services.printing.PrintTargetGuard` classifies the destination instead, refusing link-local, multicast and `0.0.0.0` outright and making `PrinterTool.dangerous(args)` true for a well-formed address matching neither a discovered printer nor the saved default, which is exempt because a human chose it once in Settings. `/api/mcp-servers` is the other worked example: MCP CRUD stays agent-reachable, but a `STDIO` transport is a command this JVM spawns through a bare `ProcessBuilder` — outside `HarnessSandbox` and outside the shell allowlist — so `requireMainForStdio` bounds that one field to the operator and `main`, guarding the row's effective transport on update so a body that omits `transport` cannot rewrite an existing STDIO server's command (JCLAW-1270). The `jclaw_api` gate reads `HttpUrl.encodedPath()`, never the model's string: OkHttp resolves dot-segments while parsing, so `/api/skills/x/files/../../../logs` checked raw left as `/api/logs` (JCLAW-1227) — parse first, then gate, because `%2e%2e` and a backslash normalise the same way and a `#` truncates. That normalisation is also what keeps a path from escaping `/api/` into the routes file's trailing `{controller}/{action}` catch-all, which resolves real actions for any verb the GET-only SPA route does not absorb. A workspace file is served `attachment` with `Content-Security-Policy: sandbox` unless it is a raster image, audio or PDF; the app declares no global CSP, so that response carries its own. Installer paths (Playwright Chromium, `uv run`, the JRE download) were audited and accepted 2026-08-28.
- **The release bundle has no Playwright `driver-bundle`** (Node.js for all five platforms, ~194 MB): `dropBundledPlaywrightDriver` removes it after `playBundle`. A bundle install downloads its platform's official Node.js on the first browser call (or from Settings → Browser), verified against the SHA-256 pinned in `services.browser.PlaywrightNode`, and hands it to Playwright as `PLAYWRIGHT_NODEJS_PATH` in `Playwright.create`'s env map; dev, tests and `play run` keep `driver-bundle` on the classpath, and the Docker image bakes in neither the driver nor Chromium — it downloads both on first use like a bundle, onto the data volume (`PLAYWRIGHT_BROWSERS_PATH=/app/data/pw-browsers`) so an image upgrade does not re-fetch them. Measured 2026-09-24: Playwright 1.63's `install chromium` works at container runtime on the Ubuntu 26.04 base on both arm64 and amd64, and the result launches; an older Playwright refused `ubuntu26.04-arm64`, which is why a separate Chromium stage once existed. Because that volume outlives Playwright bumps, the "is Chromium installed" check is revision-exact (`BrowserSetup.chromiumInstalled`, the `INSTALLATION_COMPLETE` markers at `browsers.json`'s revisions) — never "any `chromium*` directory", which would skip installing a new revision. `PlaywrightNode.VERSION` and its hashes must track the node inside `driver-bundle`, byte for byte — `PlaywrightNodeTest.thePinnedVersionIsTheNodeThatDriverBundleShips` fails a Playwright bump until they do. The chat's progress bar starts from the `Using tool: browser` status frame, because the `tool_call` frame arrives only after the call returns.
...
- `DangerousActionGate.hasStandingGrant` is an OR over a never-pruned static set and the `tool_approval_grant` table, so deleting the row alone can only fail open: every revoke path pairs `ToolApprovalGrant.revoke` with `DangerousActionGate.revokeGrant` (the endpoint) or `revokeGrantsForAgent` (the delete cascade, where the rows go by `ON DELETE CASCADE`). The row records the granting channel and peer as provenance only; the gate never reads them (JCLAW-1226). `buildPrompt` and `buildSlackPrompt` escape per grammar rather than once: Telegram needs `& < >`, Slack needs `SlackMarkdownFormatter.escape` plus a zero-width space splitting backtick runs, or a model-controlled argument closes the code fence and renders the rest of the approval prompt as mrkdwn. `*` and `_` stay unescaped — Slack publishes no escape for them, and what matters is the block structure (JCLAW-1231).
- A model-chosen network destination is screened with `SsrfGuard.isBlockedForProvider`, not `isUnsafe`: the strict guard blocks loopback and RFC-1918, which is where self-hosted inference and every LAN printer live. A per-attempt failure string returned to the model must not carry the transport exception — "refused" versus "timed out" is a port scan — so `PrintDispatcher` emits a fixed phrase per protocol and keeps the detail on `EventLogger.warn` (JCLAW-1229). A URL the **page** supplies — a `Location`, a harvested href, a sitemap entry, the browser's own final URL — is parsed with `utils.Urls`, which percent-encodes what `URI` rejects and every browser sends raw after the authority; one an operator or the model typed stays on `URI.create`, so a typo still fails at the edge with a message naming it (JCLAW-1287).
