...
do_dist() {
    ...
    # Resolve runtime JARs into lib/ before precompile so any deps the
    # generated bytecode references are present, AND so lib/ is populated
    # for the append step below — Play 1.x's deployment.textile lists
    # lib/ as required runtime content alongside precompiled/, conf/,
    # public/. Cold ivy cache: ~30s. Warm: ~2s.
    echo "==> Resolving runtime dependencies (play deps --sync)..."
    play deps --sync
