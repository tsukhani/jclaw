...
Detailed prompt templates in `./commands/`. Invoke by describing the workflow or using the skill name.
...
- For text-heavy architecture overviews (card content matters more than topology): read `./references/../templates/architecture.html`
- For flowcharts, sequence diagrams, ER, state machines, mind maps, class diagrams, C4: read `./references/../templates/mermaid-flowchart.html`
- For data tables, comparisons, audits, feature matrices: read `./references/../templates/data-table.html`
- For slide deck presentations (when `--slides` flag is present or `/generate-slides` is invoked): read `./references/../templates/slide-deck.html` and `./references/slide-patterns.md`
...
**AI-generated illustrations (optional).** If [surf-cli](https://github.com/nicobailon/surf-cli) is available, you can generate images via Gemini and embed them in the page for creative, illustrative, explanatory, educational, or decorative purposes. Check availability with `which surf` using the `exec` tool. If available:
...
- Source Serif Pro + Source Code Pro (academic, authoritative)
- Work Sans + Space Mono (modern, balanced)
- Newsreader + Inconsolata (sophisticated, journalistic)
...
- Navy + coral (`#0f172a`, `#f97316`) — confident, vibrant
- Forest + wheat (`#166534`, `#fbbf24`) — natural, organic
- Plum + mint (`#581c87`, `#10b981`) — creative, unexpected
...
**Output location:** Write to `workspace/{agent-name}/visual-explainer/`. Create the directory if it doesn't exist using the `filesystem` tool. Use descriptive filenames based on content: `jclaw-architecture.html`, `pipeline-flow.html`, `schema-overview.html`, etc.

**Open in browser:**
- macOS: `open workspace/{agent-name}/visual-explainer/filename.html`
- Linux: `xdg-open workspace/{agent-name}/visual-explainer/filename.html`

Use the `exec` tool to run the appropriate command.

**Tell the user** the file path so they can re-open or share it. Also provide a brief summary of the diagram's content and structure to give context.
...
**Text-heavy overviews (under 15 elements):** CSS Grid with explicit row/column placement. Sections as rounded cards with colored borders and monospace labels. Vertical flow arrows between sections. The reference template at `./references/../templates/architecture.html` demonstrates this pattern. Use when cards need descriptions, code references, tool lists, or other rich content that Mermaid nodes can't hold.
...
Use a real `<table>` element — not CSS Grid pretending to be a table. Tables get accessibility, copy-paste behavior, and column alignment for free. The reference template at `./references/../templates/data-table.html` demonstrates all patterns below.
...
**When to use:** A visual page explaining an essay might use a lead paragraph for the thesis, then cards for key arguments. A README visualization might use callout boxes for warnings but otherwise stay card/table-focused. Technical documentation can use callout boxes for important notes, warnings, or tips that need special attention.
...
**Before generating slides**, read `./references/slide-patterns.md` (engine CSS, slide types, transitions, nav chrome, presets) and `./references/../templates/slide-deck.html` (reference template showing all 10 types). Also read `./references/css-patterns.md` for shared patterns and `./references/libraries.md` for Mermaid/Chart.js theming.
...
Every diagram is a single self-contained `.html` file. No external assets except CDN links (fonts, optional libraries). This ensures portability and makes sharing easy. Structure:
...
- **Mobile responsiveness**: Test with narrow viewport widths to ensure content reflows appropriately on mobile devices.
- **Accessibility**: Ensure proper contrast ratios between text and background, include appropriate alt text for images, and use semantic HTML for screen readers.
- **Cognitive load**: Evaluate whether the diagram effectively simplifies complex information without overwhelming the viewer.
