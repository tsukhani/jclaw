...
**User:** "I just met a new contact, here is his business card." [image attached]

**Agent:**
1. Runs OCR on the workspace image path → extracts text (treated as untrusted data).
2. Parses fields → `full_name: "[PERSONAL_NAME]"`, `phone: "[PHONE_NUMBER]"`, `email: "[EMAIL]"`, `company_name: "Example Sdn Bhd"`, `date_added` from the `datetime` tool.
...
**Agent (to User):** "Done lah! I've added [PERSONAL_NAME] (Example Sdn Bhd) to your contacts sheet, and — with your go-ahead — emailed and WhatsApp'd him a hello. 🪷"
