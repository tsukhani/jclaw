...
class LlmStreamTransportReleaseTest extends UnitTest {
    ...
    void aStalledStreamReleasesItsCallerAndItsSocket() throws Exception {
        ...
        // The abort budget is read from the config table at dispatch; one second is the
        // smallest it goes, and the first five-second sweep is past it. The config table is
        // shared and play1 runs test classes concurrently, so the window is one call.
        ConfigService.set("llm.breaker.stall-abort-seconds", "1");
        try {
            ...
            provider.chatStream("x", List.of(ChatMessage.user("hi")), null,
                    ...
                    null, null, null);
            ConfigService.delete("llm.breaker.stall-abort-seconds");
