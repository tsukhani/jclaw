...
class WebhookTelegramControllerTest extends FunctionalTest {
    ...
    void reactionUpdateWithNotifyOffReturns200WithoutDispatch() {
        ...
        services.ConfigService.set("telegram.reactions.notify", "off");
        try {
            var bindingId = seedBinding(true);
    ...
    // JCLAW-377 per-topic agent routing is no longer a webhook-local decision: JCLAW-1231
    // collapsed both transports' copies into channels.TelegramInboundTurn.resolveTopicAgent,
    // covered by TelegramPollingRunnerTest.
