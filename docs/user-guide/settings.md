...
- **Hugging Face token** — enables the higher-accuracy **pyannote community-1 sidecar** (Python, managed automatically via `uv`), which finds the number of speakers by itself. The model is gated: accept its conditions on the Hugging Face model page first. If a token is already set under Image Generation it is **reused automatically** — the field shows "using the Image Generation token". Without any token (or without `uv`), the built-in sherpa-onnx engine runs instead, and any sidecar failure falls back to it — a diarization never fails because the sidecar couldn't run. Diarization requires `uv` and a Hugging Face token (the gated community-1 model) — without them, diarize requests return a configuration error naming what to set up. Enrolling a voice via the extract-clips flow stores several reference clips per person (cut from different parts of the recording) and naming matches against their averaged voiceprint, with one-name-one-speaker exclusivity. On the sidecar path, cross-talk regions additionally get **overlap re-attribution** (`transcription.diarization.overlapReattribution`, default on): overlapped turns are source-separated (MossFormer2) and re-checked against each speaker's voiceprint, so words spoken underneath the dominant voice attribute to the person who actually said them. Contested turns also get an **MSDD second opinion** (`transcription.diarization.msddSecondOpinion`, default on) — NeMo's frame-level speaker tracker resolves soft overlapped speech that voiceprint comparisons misjudge; turns neither instrument can call stay marked `(cross-talk?)`. Backchannel interjections spoken **under** the other voice ("Yeah", "Mm") are recovered from the separation stems as their own `(under-speech)` turns (`transcription.diarization.underSpeechRecovery`, default on).
