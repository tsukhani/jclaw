...
- **Hugging Face token** — enables the **pyannote community-1 sidecar** (Python, managed automatically via `uv`), which finds the number of speakers by itself. The model is gated: accept its conditions on the Hugging Face model page first. If a token is already set under Image Generation it is **reused automatically** — the field shows "using the Image Generation token". Diarization requires `uv` and a Hugging Face token — without them, diarize requests return a configuration error naming what to set up. Enrolling a voice via the extract-clips flow stores several reference clips per person (cut from different parts of the recording) and naming matches against their averaged voiceprint, with one-name-one-speaker exclusivity. The local pipeline is deliberately simple — pyannote segmentation plus whisper transcription with overlap-based speaker assignment — optimized for speed and privacy; for maximum attribution quality on difficult recordings, send the audio to an audio-capable chat model instead (pick a model with the Audio badge and ask for a speaker-labeled transcript).
