...
- **Diarization** — who-said-what transcripts are produced by an **audio-capable cloud chat model**: turn the subsection on, pick a provider (OpenAI or OpenRouter, using the API keys from LLM Providers) and one of its audio-capable models (the picker lists only models that accept audio input — the same ones showing an "Audio" badge in the chat model picker). The recording is sent to that model with a verbatim-diarization prompt; tell the agent who the speakers are ("the host is Anthony") and the transcript uses real names. Ordinary voice-note transcription stays local (whisper) and works without any of this. A third option, **Local Qwen2-Audio (on-device)**, runs diarization fully offline on this machine (Apple Silicon uses a 4-bit MLX build, NVIDIA uses CUDA; ~6.6 GB, downloads on first use, loads in seconds and stays resident). Recordings are processed in short chunks with your enrolled voice references re-sent per chunk so names stay consistent; expect lower transcription accuracy than the cloud models — it is the privacy-first choice. If the local model fails and a fallback cloud pair is configured (`transcription.diarization.provider.fallback` / `.model.fallback`), the tool falls back and says which engine ran.
