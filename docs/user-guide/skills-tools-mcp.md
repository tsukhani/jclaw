...
- **The circuit breaker** reacts to a server that is still connected but not answering — the case the watchdog cannot see. Every tool call has a timeout: 30 s on a connection the watchdog re-established, but two minutes on the connection made when the server was first started, which gives a cold server time to warm its caches and is the one most servers stay on. Three timeouts (or other server-side failures) in a row, or half of the last ten calls, open the server's breaker; for the next 30 s every call to it fails in microseconds with an error the agent can read — `MCP server 'name' is failing fast: too many recent tool-call failures` — instead of costing another timeout each. Two probe calls then decide whether it closes again. Each server has its own breaker, so one hung server never slows a healthy one.
