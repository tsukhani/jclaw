...
- No `User` model; AuthCheck admits a single system principal. Multi-tenancy is anticipated but not built.
- No multi-JVM deployment scenario in scope before v1.0.
...
| Session/auth caching | DB-backed via AuthCheck (current shape) | Multi-tenancy not built; session count is low |
...
1. **Hosted JClaw / multi-tenant SaaS.** Once a single deployment serves many organizations across multiple JVMs, intra-JVM fan-out and per-JVM caches both break. Redis (or Kafka, or Postgres LISTEN/NOTIFY) becomes a real candidate.
2. **Horizontal scale-out of the same tenant.** If a single operator deployment needs more than one JVM (CPU-bound LLM throughput, HA failover with active-active), cross-instance coordination is required.
