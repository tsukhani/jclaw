...
/** Conversation label for sidebar rows — preview takes precedence, falling back to agent name. */
function recentLabel(c: { preview?: string | null, agentName?: string | null, id: number }) {
        ...
        >
          ...
          >
            <div class="relative">
              ...
              <!--
                Caret toggle for the Conversations row. Sits inside the row's
                relative wrapper so the NuxtLink stays a clean link element
                (no nested interactive children), while the caret controls
                the Recents sublist via aria-expanded/controls. Hidden when
                there are no recents to toggle.
              -->
              <button
                v-if="item.to === '/conversations' && recentConversations && recentConversations.length"
                type="button"
                class="absolute right-2 top-1/2 -translate-y-1/2 p-1 text-fg-muted hover:text-fg-strong rounded transition-colors"
                :aria-expanded="recentsOpen"
                aria-controls="sidebar-recents"
                :title="recentsOpen ? 'Collapse recent conversations' : 'Expand recent conversations'"
                @click.stop="recentsOpen = !recentsOpen"
              >
                <svg
                  class="w-3.5 h-3.5 transition-transform"
                  :class="recentsOpen ? '' : '-rotate-90'"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                ><path
            ...
            <!--
              Inject the sidebar Recents list under the "Conversations" nav
              link. Each recent deep-links to /chat?conversation=ID, which the
              chat page picks up in its deepLinkConvoId watcher and opens the
              conversation inline (no routing churn). Wrapped in a collapse
              toggled by the caret above.
            -->
            <div
              v-if="item.to === '/conversations' && recentsOpen && recentConversations && recentConversations.length"
              id="sidebar-recents"
              role="list"
