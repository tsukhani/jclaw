...
describe('Skills page — SSE skill.promote_failed surfaces dragError', () => {
  it('renders the dragError banner with the failure reason when skill.promote_failed lands', async () => {
    ...
    // Replace useEventBus globally for this test. The helper is auto-imported
    // by Nuxt; we monkey-patch via the global registry.
    ...
    // The auto-imported useEventBus is available globally as `useEventBus`.
    // eslint-disable-next-line @typescript-eslint/no-explicit-any -- Reason: patch the auto-imported composable globally for this test only.
    const original = (globalThis as any).useEventBus
    // eslint-disable-next-line @typescript-eslint/no-explicit-any -- Reason: same — restoring the original at the end of the test.
    ;(globalThis as any).useEventBus = (): Bus => eventBusStub
    ...
    try {
      ...
      // Page must have registered the promote_failed handler.
      const promoteFailed = handlers.get('skill.promote_failed')
      expect(promoteFailed).toBeTruthy()

      // Fire the SSE event payload — the page should surface the error banner.
      promoteFailed!({ skillName: 'broken-skill', error: 'YAML parse failed' })
      await flushPromises()

      expect(component.text()).toContain('YAML parse failed')
      // Dismiss button has title="Dismiss".
      const dismissBtn = component.findAll('button[title="Dismiss"]').find(b => b.text() === '×')
      expect(dismissBtn).toBeTruthy()
      await dismissBtn!.trigger('click')
      await flushPromises()
      expect(component.text()).not.toContain('YAML parse failed')
    }
    finally {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any -- Reason: restore for adjacent tests.
      ;(globalThis as any).useEventBus = original
    }
  })
  ...
  it('renders an info banner when skill.promote_noop fires with a reason', async () => {
    ...
    type Bus = { onEvent: (name: string, handler: (data: unknown) => void) => void }
    const eventBusStub = {
      onEvent(n: string, h: (d: unknown) => void) {
        handlers.set(n, h)
      },
    }
    // eslint-disable-next-line @typescript-eslint/no-explicit-any -- Reason: same pattern as above; restored in finally.
    const original = (globalThis as any).useEventBus
    // eslint-disable-next-line @typescript-eslint/no-explicit-any -- Reason: stubbing the auto-imported composable to capture handlers.
    ;(globalThis as any).useEventBus = (): Bus => eventBusStub
    ...
    try {
      ...
      const noop = handlers.get('skill.promote_noop')
      expect(noop).toBeTruthy()
      noop!({ skillName: 'idempotent', reason: 'Nothing changed — same content' })
      await flushPromises()

      expect(component.text()).toContain('Nothing changed — same content')
    }
    finally {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any -- Reason: restore globals so adjacent tests use the real composable.
      ;(globalThis as any).useEventBus = original
    }
  })
})
