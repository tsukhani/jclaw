...
describe('ChatCostSection (JCLAW-28)', () => {
  ...
  it('renders a Total row at the bottom of the per-model table', async () => {
    registerEndpoint('/api/metrics/cost', () => ({
      since: '2026-04-10T00:00:00Z',
      rows: [
        { timestamp: '2026-05-09T12:00:00Z', agentId: 1, channelType: 'web',
          usageJson: makeUsage({ prompt: 100, completion: 50,
            modelId: 'model-a', modelProvider: 'p',
            promptPrice: 1, completionPrice: 2 }) },
        { timestamp: '2026-05-09T13:00:00Z', agentId: 1, channelType: 'web',
          usageJson: makeUsage({ prompt: 200, completion: 100,
            modelId: 'model-b', modelProvider: 'p',
            promptPrice: 1, completionPrice: 2 }) },
      ],
    }))
    const wrapper = await mountSuspended(ChatCostSection, {
    ...
    await flushPromises()
    const rows = wrapper.findAll('tbody tr')
    // 2 model rows + 1 total row = 3 tbody rows.
    expect(rows.length).toBe(3)
    const totalRow = rows[rows.length - 1]!
    expect(totalRow.text()).toContain('Total')
    // 100 + 200 = 300 prompt tokens summed across both rows.
    expect(totalRow.text()).toContain('300')
    // 50 + 100 = 150 completion tokens.
    expect(totalRow.text()).toContain('150')
  })

  it('Total row sums only paid models (matches table contents)', async () => {
    // Distinct from the comprehensive summary row at the top: the totals
    // row at the bottom of the table sums across the visible (paid) rows,
    // not the comprehensive paid-plus-free count. Free-tier turns DO
    // count toward the summary row but NOT toward the table total.
    registerEndpoint('/api/metrics/cost', () => ({
      since: '2026-04-10T00:00:00Z',
      rows: [
        // 1 paid turn, 100+50 tokens
        { timestamp: '2026-05-09T12:00:00Z', agentId: 1, channelType: 'web',
          usageJson: makeUsage({ prompt: 100, completion: 50,
            modelId: 'paid-model', modelProvider: 'p',
            promptPrice: 1, completionPrice: 2 }) },
        // 1 free-tier turn, 999+888 tokens — counts toward summary, NOT
        // toward the in-table total.
        { timestamp: '2026-05-09T13:00:00Z', agentId: 2, channelType: 'web',
          usageJson: JSON.stringify({
            prompt: 999, completion: 888, total: 1887,
            reasoning: 0, cached: 0, durationMs: 100,
            modelId: 'free-model',
          }) },
      ],
    }))
    ...
    expect(totalRow.text()).toContain('Total')
    // Sums paid only: 1 turn, 100 prompt, 50 completion.
    expect(totalRow.text()).toMatch(/Total[\s\S]*?\b1\b/) // 1 turn
    expect(totalRow.text()).toContain('100')
    expect(totalRow.text()).toContain('50')
    // 999 (free-tier prompt) must NOT be in the totals row.
    expect(totalRow.text()).not.toContain('999')
    expect(totalRow.text()).not.toContain('888')
  })

  it('Total row stays at the bottom regardless of sort order', async () => {
    // Sort order applies to the model rows; the Total row is structurally
    // the last <tr> in tbody and is rendered separately from the v-for.
    registerEndpoint('/api/metrics/cost', () => ({
      since: '2026-04-10T00:00:00Z',
      rows: [
        { timestamp: '2026-05-09T12:00:00Z', agentId: 1, channelType: 'web',
          usageJson: makeUsage({ prompt: 100, completion: 50,
            modelId: 'a-model', modelProvider: 'p',
            promptPrice: 1, completionPrice: 2 }) },
        { timestamp: '2026-05-09T13:00:00Z', agentId: 1, channelType: 'web',
          usageJson: makeUsage({ prompt: 200, completion: 100,
            modelId: 'z-model', modelProvider: 'p',
            promptPrice: 1, completionPrice: 2 }) },
      ],
    }))
    const wrapper = await mountSuspended(ChatCostSection, {
    ...
    await flushPromises()
    // Default sort: Cost desc → z-model first.
    let rows = wrapper.findAll('tbody tr')
    expect(rows.at(-1)!.text()).toContain('Total')
    // Sort by Model ascending → a-model first; Total still last.
    const modelHeader = wrapper.findAll('thead button').find(b => b.text().includes('Model'))!
    await modelHeader.trigger('click')
    await flushPromises()
    rows = wrapper.findAll('tbody tr')
  ...
  it('Total row is hidden in the all-free-tier empty state', async () => {
    // When the table is replaced by the all-free-tier message, no
    // <table> renders at all — so no totals row either.
    registerEndpoint('/api/metrics/cost', () => ({
      since: '2026-04-10T00:00:00Z',
      rows: [
        { timestamp: '2026-05-09T12:00:00Z', agentId: 1, channelType: 'web',
