...
// ─────────────────────────── Calendar projection ────────────────────────────
// Project every fire a task makes in a half-open window [from, to). CRON tasks
// use a limited cron expander; INTERVAL steps from nextRunAt; SCHEDULED /
// IMMEDIATE pin to their nextRunAt date.
...
/**
 * Lightweight cron expander covering the patterns humanCron recognizes. For
 * each minute slot in [from, to), tests whether the cron expression's fields
 * match. Iteration is bounded (~60k for a 42-day grid), with a 45-day cap as a
 * defensive guard against a degenerate window.
 */
function expandCron(expr: string, from: Date, to: Date): Date[] {
  ...
  // contributes its trailing 5 fields, but only when sec=0 (the grid is
  // minute-resolution); anything else can't be expanded here.
