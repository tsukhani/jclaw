...
// Mirror of VideoUnderstandingDispatcher.strategyFor.
...
<template>
  <div>
    ...
    <div class="mb-6 space-y-4">
      ...
      <p class="text-xs text-fg-muted">
        Uploaded videos are interpreted one of three ways, chosen automatically from your default
      ...
      <!-- Which strategy the default ("main") agent's model would use. -->
      <div class="px-3 py-2 text-[11px] border bg-muted border-border text-fg-muted">
        <template v-if="!mainAgent">
          No “main” agent configured yet — set one up to see how videos would be handled.
        </template>
        <template v-else-if="defaultVideoStrategy === 'native'">
          ...
          frames are sent as a native video part.
        </template>
        <template v-else-if="defaultVideoStrategy === 'multiImage'">
          ...
          sampled frames are sent as still images.
        </template>
        <template v-else>
          Your default model (<span class="font-mono">{{ mainAgent.modelId }}</span>) is text-only —
          frames are captioned into a text summary.
        </template>
      </div>
