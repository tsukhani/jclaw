...
const compactionReserveTokens = computed(() => {
  const entries = configData.value?.entries ?? []
  return entries.find(e => e.key === 'chat.compactionReserveTokens')?.value ?? '15000'
})

const compactionMinTurns = computed(() => {
  const entries = configData.value?.entries ?? []
  return entries.find(e => e.key === 'chat.compactionMinTurns')?.value ?? '10'
})

const compactionKeepMessages = computed(() => {
  const entries = configData.value?.entries ?? []
  return entries.find(e => e.key === 'chat.compactionKeepMessages')?.value ?? '10'
})

const jtokkitSafetyMultiplier = computed(() => {
  const entries = configData.value?.entries ?? []
  return entries.find(e => e.key === 'jtokkit.safetyMultiplier.unmatched')?.value ?? '1.4'
})
...
// interpretation tiers the default ("main") agent's model would use — Tier-1 native
// video / Tier-2 frames-as-images / Tier-3 text summary — computed from the same
// supportsVideo / supportsVision flags the dispatcher routes on.
...
// Mirror of VideoUnderstandingDispatcher.tierFor: video → 1, else vision → 2, else 3.
const defaultVideoTier = computed(() => {
  const m = defaultVideoModel.value
  if (m?.supportsVideo === true) return 1
  if (m?.supportsVision === true) return 2
  return 3
})
...
<template>
  <div>
    ...
    <div class="mb-6 space-y-4">
      ...
      <p class="text-xs text-fg-muted">
        Uploaded videos are interpreted in one of three tiers, chosen automatically from your default
        model's capabilities. Native-video models (Qwen-VL) receive the sampled frames as a video; vision
        models receive them as a sequence of still images; text-only models receive a written
        <span class="text-fg-muted">[hh:mm:ss]</span> summary generated by captioning each frame.
      </p>

      <!-- Which tier the default ("main") agent's model would use. -->
      <div class="px-3 py-2 text-[11px] border bg-muted border-border text-fg-muted">
        <template v-if="!mainAgent">
          No “main” agent configured yet — set one up to see which tier applies.
        </template>
        <template v-else-if="defaultVideoTier === 1">
          Your default model (<span class="font-mono">{{ mainAgent.modelId }}</span>) supports native video —
          videos use <span class="text-fg-strong">Tier&nbsp;1</span> (frames sent as a native video part).
        </template>
        <template v-else-if="defaultVideoTier === 2">
          Your default model (<span class="font-mono">{{ mainAgent.modelId }}</span>) supports vision —
          videos use <span class="text-fg-strong">Tier&nbsp;2</span> (sampled frames sent as still images).
        </template>
        <template v-else>
          Your default model (<span class="font-mono">{{ mainAgent.modelId }}</span>) is text-only —
          videos use <span class="text-fg-strong">Tier&nbsp;3</span> (frames captioned into a text summary).
        </template>
      </div>
      ...
      >
        <span class="flex-1">
          ...
          <span class="block text-[11px] text-fg-muted mt-0.5">
            ...
            cost — every frame is sent to the model (and, for text-only models, captioned with its own
            call). Long videos are sampled at this density up to this ceiling.
          </span>
        </span>
        <input
