...
<template>
  <div class="absolute inset-0 flex">
    <!--
      Pinned to main's edges so it fills whatever height main has: a status banner above
      main shrinks it, and the old viewport-height calc overflowed main by the banner's
      height, leaving a hidden scroll range and the composer's bottom off-screen.
    -->
    <!--
      ...
      <div class="flex-1 flex flex-col min-h-0">
        ...
        >
          ...
          >
            ...
            <NuxtLink
              v-if="subagentTranscript.parentConversationId"
              :to="`/chat?conversation=${subagentTranscript.parentConversationId}`"
              data-testid="subagent-transcript-back"
              class="ml-auto shrink-0 underline-offset-2 hover:underline"
            >
