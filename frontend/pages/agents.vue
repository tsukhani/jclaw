...
<template>
  <div>
    <div class="flex items-center justify-between mb-6">
      <h1 class="text-lg font-semibold text-fg-strong">Agents</h1>
    ...
    <div v-if="!providers.length && !editing && !creating"
      ...
      No LLM providers configured. Go to <NuxtLink to="/settings" class="text-fg-strong underline">Settings</NuxtLink> and add an API key first.
    </div>

    <!-- Main Agent section -->
    <div v-if="!editing && !creating" class="mb-6 space-y-2">
      <h2 class="text-sm font-medium text-fg-muted">Main Agent</h2>
      <p class="text-xs text-fg-muted">The built-in singleton agent. Always enabled, cannot be renamed or deleted. Handles admin chat and acts as the fallback route for channels without an explicit binding.</p>
    ...
    <div v-if="!editing && !creating" class="mb-6 space-y-2">
      <h2 class="text-sm font-medium text-fg-muted">Custom Agents</h2>
      <p class="text-xs text-fg-muted">Additional agents you create for specific channels, peers, or workflows.</p>
      <div class="bg-surface-elevated border border-border">
        ...
        <div v-if="!customAgents.length" class="px-4 py-8 text-center text-sm text-fg-muted">
    ...
    <div v-if="editing || creating" class="space-y-4">
      <button @click="cancel" class="text-xs text-neutral-500 hover:text-fg-strong transition-colors">&larr; Back to agents</button>
      <div class="bg-surface-elevated border border-border p-4">
        <h2 class="text-sm font-medium text-fg-strong mb-4">{{ creating ? 'New Agent' : 'Edit Agent' }}</h2>
        <div class="grid grid-cols-2 gap-3">
          <div>
            <label class="block text-xs text-neutral-500 mb-1">
              Name
              <span v-if="editing?.isMain" class="ml-1 text-fg-muted">(locked)</span>
            </label>
      ...
      <!-- Queue Mode -->
      <div v-if="editing" class="bg-surface-elevated border border-border p-4">
      ...
      <!-- Tools -->
      <div v-if="editing" class="bg-surface-elevated border border-border">
            ...
            <div class="divide-y divide-border">
              <div v-for="tool in group.tools" :key="tool.name"
                ...
                <div class="w-8 h-8 rounded flex items-center justify-center shrink-0"
                  ...
                  <svg class="w-4 h-4" :class="getToolMeta(tool.name)?.iconColor ?? 'text-fg-muted'"
        ...
        <div v-if="!agentTools.length" class="px-4 py-4 text-xs text-fg-muted text-center">
          No tools registered
        </div>
      </div>

      <!-- Exec privileges (main agent only) -->
      <div v-if="editing && editing.isMain" class="bg-surface-elevated border border-border">
      ...
      <div v-if="editing" class="bg-surface-elevated border border-border">
        ...
        <div v-if="!agentSkills.length" class="px-4 py-4 text-xs text-fg-muted text-center">
          No skills available
        </div>
      </div>
      ...
      -->
      <div v-if="editing && effectiveAllowlist" class="bg-surface-elevated border border-border">
          ...
          <table class="w-full text-xs">
            ...
            <tbody class="divide-y divide-border">
              <tr v-for="cmd in effectiveAllowlist.global" :key="'g:' + cmd">
                <td class="py-1 pr-4 font-mono text-fg-primary">{{ cmd }}</td>
              ...
              <template v-for="(cmds, skillName) in effectiveAllowlist.bySkill" :key="skillName">
                <tr v-for="cmd in cmds" :key="skillName + ':' + cmd">
                  ...
                  <td class="py-1 text-neutral-500">Skill: <span class="font-mono text-fg-muted">{{ skillName }}</span></td>
                </tr>
              </template>
            </tbody>
          </table>
    ...
    <div v-if="promptBreakdownOpen"
      ...
      <div class="bg-surface-elevated border border-border w-full max-w-4xl my-6 text-fg-primary">
        <div class="px-4 py-3 border-b border-border flex items-center justify-between gap-3">
          <div class="min-w-0">
            <h3 class="text-sm font-medium text-fg-strong truncate">System prompt — {{ promptBreakdownAgent?.name }}</h3>
          ...
          <div class="flex items-center gap-2 shrink-0">
            ...
            <button @click="closePromptBreakdown()" class="p-1 text-neutral-500 hover:text-fg-strong" title="Close">
        ...
        <div v-else-if="promptBreakdownData" class="px-4 py-4 space-y-5">
          ...
          <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs">
            <div class="bg-muted border border-border px-3 py-2">
              ...
              <div class="text-sm font-mono text-fg-strong">{{ formatChars(promptBreakdownData.totalChars) }}</div>
            ...
            <table class="w-full text-xs font-mono">
              ...
              <tbody>
                <tr v-for="s in promptBreakdownData.sections" :key="'section-' + s.name"
                  ...
                  <td class="py-1 pr-2 text-fg-primary">{{ s.name }}</td>
                  <td class="py-1 px-2 text-right text-fg-muted">{{ formatChars(s.chars) }}</td>
          ...
          <div v-if="promptBreakdownData.tools.length > 0">
            ...
            <p class="text-[10px] text-fg-muted mb-1">Sent separately as the <code class="text-neutral-500">tools</code> array, not part of the prompt string, but counted as input tokens by every provider.</p>
            <table class="w-full text-xs font-mono">
              <thead class="text-[10px] text-neutral-500 border-b border-border">
                <tr>
              ...
              <tbody>
                <tr v-for="t in promptBreakdownData.tools" :key="'tool-' + t.name"
                    class="border-b border-neutral-900/50">
                  <td class="py-1 pr-2 text-fg-primary">{{ t.name }}</td>
                  <td class="py-1 px-2 text-right text-fg-muted">{{ formatChars(t.chars) }}</td>
