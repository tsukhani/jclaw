...
const levelColors: Record<string, string> = {
  ...
  INFO: 'bg-neutral-200 dark:bg-neutral-800 text-neutral-700 dark:text-neutral-400 border-neutral-300 dark:border-neutral-700',
}
...
<template>
  <div>
    <div class="flex items-center justify-between mb-6">
      <h1 class="text-lg font-semibold text-neutral-900 dark:text-white">Logs</h1>
    ...
    <div class="bg-neutral-50 dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800">
      <div v-if="logs?.events?.length" class="divide-y divide-neutral-200 dark:divide-neutral-800/50">
        <div
          ...
          class="px-4 py-2 hover:bg-neutral-100 dark:hover:bg-neutral-800/30 transition-colors"
        >
          <div class="flex items-start gap-3">
            <span class="text-xs text-neutral-400 dark:text-neutral-600 shrink-0 w-20 font-mono mt-0.5">
            ...
            <span class="text-sm text-neutral-700 dark:text-neutral-300 min-w-0">{{ event.message }}</span>
          </div>
          <div v-if="expandedId === event.id && event.details"
               class="mt-2 ml-[8.5rem] text-xs font-mono text-neutral-500 bg-neutral-100 dark:bg-neutral-800/50 p-2 whitespace-pre-wrap">
            {{ event.details }}
          </div>
        </div>
      </div>
