...
public class ConversationService {
    ...
    public static boolean requestTitleGeneration(Conversation conversation) {
        // Idempotency gate: a conversation's title is generated at most
        // MAX_TITLE_GENERATIONS times — an initial pass after the opening
        // turn, then one refresh once the conversation has grown. Without
        // this bound, the chat page fires this endpoint on every sidebar
        // switch and the LLM keeps rewriting the preview, burning tokens
        // and visibly flipping the title each time.
        if (conversation.titleGenerationCount >= Conversation.MAX_TITLE_GENERATIONS) return false;
